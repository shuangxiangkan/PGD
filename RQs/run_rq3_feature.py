...
VARIANTS = [
    Variant("full", None),
    Variant("w/o match ratio", "match"),
    Variant("w/o mismatch ratio", "mismatch"),
    Variant("w/o dispersion", "dispersion"),
]
...
def collect_metrics(curve_path: Path) -> dict[str, float | int]:
    ...
    mean_f1 = statistics.mean(float(row["refined_f1"]) for row in rows)
    out: dict[str, float | int] = {
        "accuracy": float(final["refined_accuracy"]),
        "precision": float(final["refined_precision"]),
        "recall": float(final["refined_recall"]),
        "f1": float(final["refined_f1"]),
        "brier": float(final["refined_brier"]),
        "ece": float(final["refined_ece"]),
        "topk": float(final.get("refined_topk", 0.0) or 0.0),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run RQ3 node-feature ablation experiments.")
        ...
        for run_id in range(1, args.runs + 1):
            ...
            if args.force or not train_data.with_suffix(".npz").exists():
                ...
                run_command(
                    ...
                    log_dir / f"{data_stem}_generate_train.log",
                )
            if args.force or not test_data.with_suffix(".npz").exists():
                ...
                run_command(
                    ...
                    log_dir / f"{data_stem}_generate_test.log",
                )

            for variant in VARIANTS:
                variant_id = variant.name.replace("/", "without").replace(" ", "_")
                stem = f"{data_stem}_{variant_id}"
                curve_path = curve_dir / f"{stem}.csv"
                if args.force and curve_path.exists():
                    curve_path.unlink()
                if not curve_path.exists():
                    print(f"[method] {case.label} run={run_id} variant={variant.name}")
                    run_command(
                        method_command(train_data, test_data, curve_path, variant, seed, args),
                        log_dir / f"{stem}.log",
                    )
                metrics = collect_metrics(curve_path)
                rows.append(
                    {
                        ...
                        "drop_feature": variant.drop_feature or "none",
                        "run": run_id,
                        "seed": seed,
                        **metrics,
                    }
    ...
    raw_path = out_dir / "rq3_feature_raw.csv"
    topology_path = out_dir / "rq3_feature_summary_by_topology.csv"
    variant_path = out_dir / "rq3_feature_summary_by_variant.csv"
    write_csv(raw_path, rows)
    write_csv(topology_path, summarize(rows, ["profile", "topology", "variant", "drop_feature"]))
    write_csv(variant_path, summarize(rows, ["profile", "variant", "drop_feature"]))
    round_csv(raw_path, out_dir / "rq3_feature_raw_rounded.csv")
    round_csv(topology_path, out_dir / "rq3_feature_summary_by_topology_rounded.csv")
    round_csv(variant_path, out_dir / "rq3_feature_summary_by_variant_rounded.csv")
