...
def method_command(train_data: Path, test_data: Path, curve_path: Path, seed: int, args) -> list[str]:
    cmd = [
        str(PYTHON),
...
def collect_metrics(curve_path: Path, metric_prefix: str) -> dict[str, float | int]:
    rows = read_curve(curve_path)
    if not rows:
        raise ValueError(f"empty curve file: {curve_path}")
    f1_key = f"{metric_prefix}_f1"
    final = rows[-1]
    best = max(rows, key=lambda row: float(row[f1_key]))
    first_loss = float(rows[0]["train_loss"])
    final_loss = float(final["train_loss"])
    out: dict[str, float | int] = {
        metric: float(final[f"{metric_prefix}_{metric}"]) for metric in FINAL_METRICS
    }
    out.update(
        {
            "best_f1": float(best[f1_key]),
            "best_f1_epoch": int(float(best["epoch"])),
            "epoch_to_f1_095": first_epoch_at(rows, f1_key, 0.95),
            "epoch_to_f1_099": first_epoch_at(rows, f1_key, 0.99),
            "mean_f1_over_epochs": statistics.mean(float(row[f1_key]) for row in rows),
            "final_train_loss": final_loss,
            "loss_reduction": first_loss - final_loss,
        }
    )
    for metric in REFINEMENT_METRICS:
        out[metric] = float(final.get(metric, 0.0) or 0.0)
    return out
...
def summarize(rows: list[dict[str, object]], group_keys: list[str]) -> list[dict[str, object]]:
    metrics = FINAL_METRICS + CONVERGENCE_METRICS + REFINEMENT_METRICS
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run RQ3 posterior-refinement ablation experiments.")
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "RQs" / "results" / "RQ3_refinement")
    ...
    for case in cases:
        for run_id in range(1, args.runs + 1):
            ...
            curve_path = curve_dir / f"{stem}_full.csv"

            if args.force or not train_data.with_suffix(".npz").exists():
                print(f"[generate] {case.label} run={run_id} train")
                run_command(dataset_command(case, train_data, seed, train_samples, args), log_dir / f"{stem}_generate_train.log")
            if args.force or not test_data.with_suffix(".npz").exists():
                print(f"[generate] {case.label} run={run_id} test")
                run_command(dataset_command(case, test_data, seed + 97, test_samples, args), log_dir / f"{stem}_generate_test.log")
            if args.force and curve_path.exists():
                curve_path.unlink()
            if not curve_path.exists():
                print(f"[method] {case.label} run={run_id}")
                run_command(method_command(train_data, test_data, curve_path, seed, args), log_dir / f"{stem}_full.log")

            for variant_name, metric_prefix in VARIANTS:
                rows.append(
                    {
                        ...
                        "run": run_id,
                        "seed": seed,
                        **collect_metrics(curve_path, metric_prefix),
                    }
                )

    raw_path = out_dir / "rq3_refinement_raw.csv"
    topology_path = out_dir / "rq3_refinement_summary_by_topology.csv"
    variant_path = out_dir / "rq3_refinement_summary_by_variant.csv"
    write_csv(raw_path, rows)
    write_csv(topology_path, summarize(rows, ["profile", "topology", "variant"]))
    write_csv(variant_path, summarize(rows, ["profile", "variant"]))
    round_csv(raw_path, out_dir / "rq3_refinement_raw_rounded.csv")
    round_csv(topology_path, out_dir / "rq3_refinement_summary_by_topology_rounded.csv")
    round_csv(variant_path, out_dir / "rq3_refinement_summary_by_variant_rounded.csv")
